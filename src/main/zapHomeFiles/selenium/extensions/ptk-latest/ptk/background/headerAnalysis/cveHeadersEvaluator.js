...
export function setCveTechnologiesForHeaders(technologies = {}) {
    try {
        cachedPlan = buildHeaderPlan(technologies) || []
    } catch (err) {
        cachedPlan = []
        console.warn("[PTK][HeaderAnalysis] Failed to build CVE header plan", err)
    }
}
...
export function evaluateCveHeaders(responses = []) {
