...
*/
var PBKDF2Algo = class extends require_core.Base {
	cfg;
	/**
	...
	constructor(cfg) {
		...
		*/
		this.cfg = Object.assign({}, {
			keySize: 128 / 32,
			hasher: require_sha256.SHA256Algo,
			iterations: 25e4
		}, cfg);
	}
	/**
	...
	*/
	compute(password, salt) {
		const { cfg } = this;
		const hmac = require_core.HMAC.create(cfg.hasher, password);
		const derivedKey = require_core.WordArray.create();
		const blockIndex = require_core.WordArray.create([1]);
...
//# sourceMappingURL=pbkdf2.cjs.map