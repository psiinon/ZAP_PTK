...
*/
var Cipher = class Cipher extends require_core.BufferedBlockAlgorithm {
...
*/
var BlockCipherMode = class extends require_core.Base {
	/** The cipher instance */
	_cipher;
...
const Pkcs7 = {
	pad(data, blockSize) {
		...
		const padding = require_core.WordArray.create(paddingWords, nPaddingBytes);
		data.concat(padding);
	},
	unpad(data) {
...
*/
var CipherParams = class CipherParams extends require_core.Base {
	ciphertext;
	key;
	iv;
	salt;
...
const OpenSSLFormatter = {
	stringify(cipherParams) {
		...
		if (salt && ciphertext) wordArray = require_core.WordArray.create([1398893684, 1701076831]).concat(salt).concat(ciphertext);
		else if (ciphertext) wordArray = ciphertext;
		else wordArray = new require_core.WordArray();
		return wordArray.toString(require_enc_base64.Base64);
	},
	parse(openSSLStr) {
		let salt;
		const ciphertext = require_enc_base64.Base64.parse(openSSLStr);
		const ciphertextWords = ciphertext.words;
		if (ciphertextWords[0] === 1398893684 && ciphertextWords[1] === 1701076831) {
			salt = require_core.WordArray.create(ciphertextWords.slice(2, 4));
...
*/
var SerializableCipher = class extends require_core.Base {
	/** Configuration options */
...
const OpenSSLKdf = { execute(password, keySize, ivSize, salt, hasher) {
	let _salt;
	if (!salt) _salt = require_core.WordArray.random(64 / 8);
	else if (typeof salt === "string") _salt = require_core.Hex.parse(salt);
	else _salt = salt;
	let key;
	if (!hasher) key = require_evpkdf.EvpKDFAlgo.create({ keySize: keySize + ivSize }).compute(password, _salt);
	else key = require_evpkdf.EvpKDFAlgo.create({
		keySize: keySize + ivSize,
		hasher
	}).compute(password, _salt);
	const iv = require_core.WordArray.create(key.words.slice(keySize), ivSize * 4);
...
exports.OpenSSLFormatter = OpenSSLFormatter;
...
//# sourceMappingURL=cipher-core.cjs.map